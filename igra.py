# ***создаем игру с летящим шариком в квадрате и с отталкивающей пластиной
# логика алгоритма:
# 1) игра начинается
# 2) шарик начинается двигаться
# 3) если тапнуть влево или вправо - двигаем платформу
# 4) если шарик коснулся стенок, потолка или платформы - делаем отскок шарика
# 5) если шарик коснулся платформы - увеличиваем счет на единицу
# 6) если шарик упал на пол - выводим сообщение и заканчиваем игру

# все происходит параллельно и независимо друг от друга
# => нужно определить три класса - платформу, шарик и счет - также действия друг на друга
# задаем один раз, а потом объекты сами разбираются, как им реагировать друг на друга в разных ситуациях
# для задавания правил игры - классы подходят идеально

# используем библиотеку Tkinter(рисовашки)
# **********КОД**********
...
import time
import random
# создаем новый объект - окно с игровым полем. Переменная окна - tk, из класса Tk() - он есть в графической библиотеке
tk = Tk()
...
tk.wm_afcttributes("-topmost", 1)
# создаем новый холст - 400 на 500 пиксселей
canvas = Canvas(tk, width=500, height=400, highlightthickness=0)
# у каждого элемента на холсте своя координата
canvas.pack()
...
# ШАРИК должен уметь:
# задавать свое ачальное положение и направление движения
# понимать момент касания платформы
# рисовать сам себя и понимать, когда нужно отрисовать ссебя в новом положении
